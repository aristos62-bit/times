...
class BackupService {
  ...
  Future<File> createBackup({String? name}) async {
    ...
    final dbFile = File(p.join(dbDir.path, AppConstants.dbName));
  ...
  Future<void> restoreFromBackup(String zipPath) async {
    ...
    try {
      ...
      final dbWal = File(p.join(dbDir.path, '${AppConstants.dbName}-wal'));
      final dbShm = File(p.join(dbDir.path, '${AppConstants.dbName}-shm'));
