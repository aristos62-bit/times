...
1. Αυτόματος αύξων αριθμός απόδειξης (SPoT sequence από τη βάση, read-only πεδίο).
...
- (Προαιρετικό, βλ. Φάση 6) Διαχείριση μονάδων μέτρησης, προμηθευτών.
...
```
- `receiptNumber`: αυτόματη ακολουθία διαχειριζόμενη από τη βάση (SPoT sequence, όχι hardcoded counter στο UI).
...
3. Γραφήματα με `fl_chart`, responsive σε όλα τα μεγέθη οθόνης, dark-mode συμβατά χρώματα.
...
4. Προαιρετικά (μόνο αν το ζητήσεις): διαχείριση προμηθευτών/μονάδων από Ρυθμίσεις, cloud sync ως μελλοντική επέκταση.
