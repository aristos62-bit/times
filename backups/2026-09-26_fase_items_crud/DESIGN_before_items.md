...
**Γραφήματα (κλειδωμένα 25-09-2026 — όλα pie, σύνολα €, καθαρά §3)**
1. Ανά προμηθευτή · 2. Ανά κατηγορία · 3. Ανά υποκατηγορία · 4. Top-10 είδη (κατά σύνολο €).
...
- Κάθε κάρτα έχει δικό της selector (date-range picker στο Προσαρμοσμένο)·
  default Μήνας (τρέχων). Global φίλτρο ΔΕΝ υπάρχει (το παλιό
  `selectedPeriodProvider`/τρίμηνο ΑΠΟΣΥΡΕΤΑΙ).
- `PeriodType` (app_enums, Φάση 5): day/week/month/year/custom (+
...
- 4 streams ( `supplierTotalsStreamProvider` / `categoryTotalsStreamProvider` / `subCategoryTotalsStreamProvider` / `topItemsStreamProvider`, `.family` ανά `ChartQuery{from,to,limit}`) → repo passthrough (error-mapping μόνο) → DAO aggregations (§3).
...
**Λίστα πρόσφατων αποδείξεων (κάτω από το save — Φάση 3 Βήμα 7 + Φάση Α actions 24-09-2026)**
...
- **Φάση Α (24-09-2026) — επεξεργασία + διαγραφή**: κάθε γραμμή έχει μολύβι (`editAction` → `loadReceiptForEdit`, φόρτωση date/supplier/drafts στη φόρμα με `editingId`· γεμάτα drafts → confirm `editDiscardDraftsConfirm`) + κόκκινο κάδο (`deleteAction`, `colorScheme.error` → `showConfirmDialog(isDestructive)` + `deleteReceipt`, CASCADE §3). Save σε edit mode → `updateReceiptWithLines` (update κεφαλίδας + αντικατάσταση γραμμών, μία transaction· τα line-ids αλλάζουν, SUM §3) + `receiptUpdated`· banner `receiptNumber(editingId)` + «Ακύρωση» (`cancelEdit`). SPoT +1/+4 (`updateReceipt`, `receiptUpdated`, `receiptDeleted`, `deleteReceiptConfirm`, `editDiscardDraftsConfirm`).
...
- `price > validationMinPrice`, `quantity > validationMinQuantity` — σε λειτουργία
  ...
  Έκπτωση συνόλου (25-09-2026): `0≤D≤T` (ίδια μηνύματα — `discountTooLarge`
...
- `inUseCount*`/`receiptCount*` (int siblings για tooltip) + refresh ορατών πυλών (one-shot families, IndexedStack) — σκόπιμα one-shot αντί live watches (24-09-2026: 124 μόνιμες συνδρομές απορρίφθηκαν· staleness ακίνδυνο — defense στον controller)· auto-refresh από save/delete (25-09-2026: `_refreshAffectedGuards` — μόνο επηρεαζόμενα ids, swallow σε failure).
...
1. Δημιουργία Flutter project, ρύθμιση `pubspec.yaml` (Drift, Riverpod, GoRouter, fl_chart, shared_preferences, file_picker + path_provider + sqlite3 για backup).
...
### Φάση 5 — Κεντρική Σελίδα (4 πίτες 3D-εφέ, κλειδώθηκε 25-09-2026)
