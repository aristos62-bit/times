...
- **Επόμενα βήματα:** Φάση 0, Βήμα 4 υπόλοιπα SPoT — `app_messages.dart` επόμενο → `app_errors.dart`, `app_enums.dart`, `app_theme.dart`, `app_colors.dart`, `app_routes.dart`, `app_exceptions.dart` (το `app_exceptions.dart` έχει TODO "logging tag" — θα χρησιμοποιήσει το `LogTag` από το debug_config).
