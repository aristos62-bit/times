...
> - `import 'utc_date_time_converter.dart';` (για τα utcDateTime() columns)
...
/// Αποθήκευση: πάντα UTC (ISO8601). Ανάγνωση: πάντα local timezone.
/// Έτσι είναι ΑΔΥΝΑΤΟ να ξεχαστεί .toUtc() κάπου — γίνεται σε κάθε write.
...
/// SPoT: ΘΕΛΟΥΜΕ ΑΥΤΟ (utcDateTime) αντί για dateTime() σε ΟΛΟΥΣ τους πίνακες.
/// Με αυτό, τα monthly reports είναι σωστά ακόμα και με αλλαγή ζώνης ώρας/DST.
Column<DateTime> utcDateTime() => dateTime().map(const UtcDateTimeConverter());

// Σημ.: Επειδή το .map() δίνει ΓΕΝΙΚΟ Column<DateTime> (όχι DateTimeColumn),
// οι στήλες ημερομηνιών στα tables δηλώνονται ως:
//   Column<DateTime> get receiptDate => utcDateTime()();
```
...
class Categories extends Table {
  ...
  Column<DateTime> get createdAt => utcDateTime().withDefault(currentDateAndTime)();
  Column<DateTime> get updatedAt => utcDateTime().withDefault(currentDateAndTime)();
...
class Payments extends Table {
  ...
  Column<DateTime> get paymentDate => utcDateTime()();
...
class BackupService {
  ...
  Future<void> restoreFromBackup(String zipPath) async {
    ...
    try {
      ...
      final dbWal = File(p.join(dbDir.path, '${DatabaseConstants.dbName}-wal'));
      final dbShm = File(p.join(dbDir.path, '${DatabaseConstants.dbName}-shm'));
