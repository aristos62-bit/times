...
1. **Theme persistence** (Light/Dark/Auto): `themeModeProvider` → **Notifier** πάνω σε `SettingsRepository` (SharedPreferences, §2.0.1 SPoT keys §2.3:270) · `main.dart` γίνεται `ConsumerWidget` με `ref.watch(themeModeProvider).mode` → `MaterialApp.router.themeMode` (§2.3:255).
...
4. **SettingsPage + category tree editor** (Βήμα 4): CRUD Κατηγοριών/Υποκατηγοριών μόνο (§2.3:274-276· cascade-delete Items side-effect, **όχι** νέο UI ειδών §2.3.1:295-298) · reuse ConfirmDialog (isDestructive) + NameValidator + AppFeedback + shared widgets (§2.4).
5. **BackupService + UI** (Βήμα 5): Export = `VACUUM INTO 'path'` (customStatement, snapshot WAL-safe, §2.3:279) · file_picker save dialog (§4:257) · Restore = validate → **confirm → auto-backup** → `closeSafely()` (idempotent, νέο §2.3.1:300) → replace → **restart providers** (`ref.invalidate(appDatabaseProvider)`, §2.3:282).
