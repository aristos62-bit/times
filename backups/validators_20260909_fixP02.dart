...
class Validators {
  ...
  static String? validateReceiptDate(DateTime? date) {
    ...
    if (date.isAfter(DateTime.now())) return AppStrings.receiptDateInFuture;
  ...
  static String? validateVatRate(String? rate) {
    ...
    if (!AppConstants.vatRates.contains(parsed)) return AppStrings.invalidVatRateValue;
    return null;
  }
  ...
  static String? validatePhone(String? phone) {
    ...
    if (!RegExp(r'^(\+30)?[0-9]{10}$').hasMatch(cleaned)) return AppStrings.invalidPhoneNumber;
    return null;
  }
  
  static String? validateEmail(String? email) {
    if (email == null || email.isEmpty) return null; // Optional
    if (!RegExp(r'^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$').hasMatch(email)) return AppStrings.invalidEmail;
    return null;
  }
