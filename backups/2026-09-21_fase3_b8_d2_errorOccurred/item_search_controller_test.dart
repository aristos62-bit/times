...
void main() {
  ...
  /// ΠΡΩΤΑ ελέγχει το τρέχον value: αν ήδη ικανοποιεί, επιστρέφει άμεσα
  /// (το AsyncNotifier equality-gate δεν επανεκπέμπει ίδιο state).
  Future<ItemSearchState> waitForStatus(
    ProviderContainer container,
    bool Function(ItemSearchState s) predicate,
  ) {
    final current = container.read(itemSearchControllerProvider).value;
  ...
  group('retry', () {
    test('μετά από error → retry ξανα-τρέχει την τελευταία αναζήτηση',
      ...
      // retry μετά από found ξανα-τρέχει (still found). Περιμένουμε το
      // found (όχι searching· το searching έχει κενά results και θα
      // ικανοποιούσε άμεσα ένα combined predicate).
      notifier.onQueryChanged('Γάλα');
  ...
  group('createSubCategory', () {
    ...
    test('dup μέσα στην κατηγορία → (null, false)', () async {
      ...
      final result = await notifierOf(container)
          .createSubCategory(categoryId: categoryId, name: 'ΓΑΛΑΚΤΟΚΟΜΙΚΑ');
      expect(result.created, isFalse);
