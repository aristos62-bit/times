...
- **Περίληψη:** **App Logo "παντού"** — SPoT `assets/icons/Times.png` (1254×1254) → `flutter_launcher_icons` 0.14.4 (dev dep + pubspec config): Android legacy mipmaps (5), Windows `.ico` 256px, Web icons + favicon (χειροκίνητο), iOS AppIcon (14), macOS (7). DESIGN/design docs + folder structure synced. **Fix-C (12/09/2026):** DI service locator — `Map<Type,dynamic>` + `get<T>()` → **14 ρητά typed getters** (compile-time safety)· stale σχόλια (app.dart, receipt_bloc) corrected· +1 test «πλήρες graph» → **coverage DI 100%**, **389/389 tests, analyze clean**.