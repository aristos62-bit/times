...
## 11. Φάση 0 · Βήμα 4 SPoT — `app_exceptions.dart`

> Ημερομηνία: 14-09-2026. Append στο κεφάλαιο (η φάση 0 παραμένει ένα κεφάλαιο).

- **Γέμισμα `lib/core/errors/app_exceptions.dart`** (DESIGN §2.1-§2.4, §1.2, §1.7) — **5 custom exceptions + sealed βάση**. Ορισμός: *«Custom exception classes με structured mapping προς app_errors.dart — ποτέ raw string στα UI»* + TODO «userMessage + logging tag»:
  - **Βάση**: `sealed class AppException implements Exception` — fields `userMessage` (AppErrors) + `loggingTag` (LogTag). `sealed` = οι μόνες υλοποιήσεις σε αυτό το αρχείο → το mapping προστατεύεται **κατά compile** (κανένα raw string, κανένα «παράταιρο» exception εκτός SPoT).
  - **5 final κλάσεις** (μία = ένα AppErrors const):
    1. `SaveReceiptException` → `saveFailed` + `LogTag.db` (§2.2:210-212).
    2. `DataLoadException` → `loadDataFailed` + `LogTag.db` (§2.1 error state / §2.4 AsyncValueView).
    3. `BackupCreationException` → `backupFailed` + `LogTag.backup` (§2.3 Βήμα 1 Export **και** Βήμα 3 auto-backup — 1 κλάση, 2 σημεία, όχι διπλή).
    4. `InvalidBackupFileException` → `invalidBackupFile` + `LogTag.backup` (§2.3 Βήμα 2 file validation, καμία αλλαγή στη βάση).
    5. `RestoreBackupException` → `restoreFailed` + `LogTag.backup` (§2.3 Βήμα 4 αντικατάσταση).
  - **`toString` = `'$runtimeType: $userMessage'`** — στοχευμένο για το `| error` που καταγράφει ο AppLogger.error (app_logger.dart:57· επιβεβαιωμένο και στο app_logger_test.dart:42).
- **Layering/imports**: `errors → {constants/app_errors, debug/debug_config}` — το LogTag από το **debug_config** (όχι από το logger) σύμφωνα με το docstring debug_config:6-9. Ακυκλικό (debug→foundation μόνο). **Κανένα logging εδώ** — καθαρά δεδομένα· το logging γίνεται στο σημείο σύλληψης (repo/service, §1.7) + το AppFeedback logάρει ήδη tag UI.
- **Freezed αποκλείστηκε σκόπιμα** (βρέθηκε στους reviewers: ήδη στο pubspec + DESIGN για models/state): fields-as-parameters θα επέτρεπαν raw strings → σπάει το mapping · build_runner/codegen για 2 fields = υπερβολή · non-const χωρίς όφελος (δεν θες copyWith/==) — το toString το γράφουμε στοχευμένα.
- **NOTE(Φάση0-Βήμα4)**: validation exceptions (π.χ. `ValidationException`, §2.2:214-218) → Φάση 3 μαζί με validators (σύμφωνο NOTE app_errors). Αποκλείστηκαν επίσης: πεδίο `cause`/μέθοδοι logging στη βάση (πέραν του ρητού συμβολαίου «userMessage + loggingTag»), αλλαγές σε app_logger/app_feedback/main/app_enums/app_routes.
- **`test/core/errors/app_exceptions_test.dart`** (νέος φάκελος-καθρέφτης) — 10 tests, patterns από debug_config_test (`tearDown(DebugConfig.reset)`) + app_logger_test (`tearDown(AppLogger.resetTestSink)`):
  1. Ιεραρχία: όλα `isA<Exception>()` + `isA<AppException>()`.
  2-6. Exact mapping ανά κλάση: `AppErrors.*` **και** `LogTag` (db/backup).
  7. Loop quality: μη-κενά + `.trim()`.
  8. Loop: κάθε `loggingTag` ενεργό στο `DebugConfig.isTagEnabled`.
  9. `toString()` = `'SaveReceiptException: Σφάλμα κατά την αποθήκευση'`.
  10. **Integration (reuse `AppLogger.testSink`)**: `AppLogger.error(e.loggingTag, msg, e)` → `[DB][ERROR] ... | SaveReceiptException: ...` — αποδεικνύει ότι tag+toString λειτουργούν ΉΔΗ με τον υπάρχοντα logger.
- **Σύνολο suite:** **77/77** ✓ (67 + 10 app_exceptions) · `flutter analyze` **No issues** ✓.
- **Δεν άλλαξε DESIGN.md** — καμία αρχιτεκτονική αλλαγή (κανόνας 8).
- **Backups** (`backups/*_before_step4_20260914_185417.*`): `app_exceptions`, `oldsessions.md`, `fase0_branding.md`.

---

## 12. Φάση 0 · Βήμα 4 SPoT — `app_colors`, `app_theme`, `app_routes`, `app_enums`

> Ημερομηνία: 14-09-2026. Append στο κεφάλαιο (η φάση 0 παραμένει ένα κεφάλαιο). Ολοκληρώνει τα SPoT του Βήματος 4.

- **`lib/core/theme/app_colors.dart`** (DESIGN §0, §1.1, §1.5, §4) — **3 SPoT const** σε `abstract final class`:
  - `brandSeed = Color(0xFF00897B)` — teal seed (§0 branding, ταυτίζεται με splash pubspec §3). Ήδη σε 2+ σημεία (app_feedback_test.dart:70 χρησιμοποιούσε raw `Color(0xFF00897B)`) → trigger κανόνα §1.1.
  - `priceUp = Color(0xFFD32F2F)` / `priceDown = Color(0xFF2E7D32)` — semantic (άνοδος/πτώση τιμής, χρήστες: γραφήματα Φάσης 5 fl_chart §4· dark variants όταν χρειαστούν).
- **`lib/core/theme/app_theme.dart`** (DESIGN §1.5, §0) — theme SPoT:
  - `AppTheme.defaultMode = ThemeMode.system` — η ρητή προεπιλογή §1.5 (override Light/Dark/Auto → Φάση 4, themeModeProvider persisted §2.3).
  - `light` / `dark` — `static final` (memoized, μία αξιολόγηση) ThemeData από `ColorScheme.fromSeed(seedColor: AppColors.brandSeed, brightness: ...)`.
  - `_build(brightness)` — μία υλοποίηση, μόνο brightness διαφέρει.
- **`lib/core/router/app_routes.dart`** (DESIGN §1.1:33, §1.2:42, §2.1/§2.2/§2.3) — **3 routes** (name+path):
  - `home` + `homePath='/'` (initial route §2.1) · `priceEntry` + `priceEntryPath='/price-entry'` (§2.2) · `settings` + `settingsPath='/settings'` (§2.3).
  - Σύμβαση: paths kebab-case ξεκινώντας `/` χωρίς trailing slash (εξαίρεση: root `/`) · names camelCase μοναδικά (απαίτηση GoRouter) · **μηδέν imports** (μονό SPoT — το GoRouter config είναι ξεχωριστό αρχείο route definitions).
- **`lib/core/constants/app_enums.dart`** — **ΔΕΝ γέμισε με enum**· έμεινε δεσμευμένο με `NOTE(Φάση0-Βήμα4)`. Αιτιολόγηση από πλήρη έλεγχο DESIGN:
  - Μονάδες/κατηγορίες = **δεδομένα ΒΔ** (§3, seed Phase 1 στο `data/local/seed/`) — enum στο core θα δημιουργούσε **δεύτερη πηγή αλήθειας** (§1.1 «Καμία εξαίρεση»).
  - `PeriodType` (μήνας/τρίμηνο/έτος/custom, §2.1:140 → period_filter_bar) → **Φάση 5**· σήμερα το DESIGN δηλώνει `StateProvider<DateRange>`, μηδέν καταναλωτές.
  - `ThemeMode` = built-in Flutter (σε app_theme) · `LogTag` = ήδη στο debug_config.
  - Política: μονάδες υλοποιούνται όταν -και- υπάρξει καταναλωτής (idem precedent validation messages app_errors §10).
- **Wiring `lib/main.dart`** (εγκεκριμένο edit σε εξτρά αρχείο — αφαιρεί τη μόνη raw-χρώμα παράβαση του §1.5): αντικατάσταση του template `colorScheme: .fromSeed(seedColor: Colors.deepPurple)` → `theme: AppTheme.light` + `darkTheme: AppTheme.dark` + `themeMode: AppTheme.defaultMode`. Το purple φεύγει, μπαίνει το teal branding (§0).
- **Cleanup `test/core/utils/app_feedback_test.dart:70`**: raw `Color(0xFF00897B)` → `AppColors.brandSeed` (εξαλείφεται η 2η hardcoded εμφάνιση· το test παραμένει ίδιο).
- **Τρία νέα test files** (mirror §1.8):
  1. `test/core/theme/app_colors_test.dart` — 4: ισχύς 3 const (exact hex) + διακριτότητα.
  2. `test/core/theme/app_theme_test.dart` — 5: brightness×2 · primary == `ColorScheme.fromSeed(brandSeed, b).primary` ×2 (δεσμεύει το seed) · `defaultMode == ThemeMode.system`.
  3. `test/core/router/app_routes_test.dart` — 6: exact pairs×3 · μορφή paths (ξεκινά `/`, trailing-slash εκτός root) · μοναδικότητα names+paths · homePath='/' initial.
  - (app_enums: χωρίς test — χωρίς λειτουργία, συνέπεια κανόνα 4.)
- **Fix ανάλυσης:** 1ο `flutter analyze` → 4 issues (κλειστά): `Color` στο test (προστέθηκε `import 'dart:ui' show Color'`) + `unintended_html_in_doc_comment` στο app_enums (backticks `StateProvider<DateRange>`). 1ο `flutter test` → 1 fail στο trailing-slash check (root `/` εξαιρείται ρητά). Σταθερό **92/92**.
