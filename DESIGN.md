...
- **Κρίσιμο**: η άθροιση γίνεται **στο SQL** (`SUM(lineTotalCents) GROUP BY ...`), όχι φορτώνοντας όλες τις γραμμές στη μνήμη και αθροίζοντας σε Dart — αλλιώς σε μεγάλο ιστορικό (πολλούς μήνες) η οθόνη κολλάει. Τα σύνολα είναι καθαρά (μετά έκπτωση, §3)· οι μοναδιαίες τιμές πάντα καθαρές (Δ-stat §3).
...
ITEM_SELECTED ──▶ εμφάνιση Unit dropdown (προεπιλογή: Item.defaultUnitId αν υπάρχει)
             ...
             → Έκπτωση (προαιρετική, ανά μονάδα — ΜΟΝΟ σε unit-mode·
               τελικό γραμμής `(τιμή−έκπτωση)×ποσότητα`)
