...
void main() {
  group('AppDatabase', () {
    ...
    test('schemaVersion ταυτίζεται με AppConstants.dbVersion', () {
      expect(db.schemaVersion, AppConstants.dbVersion);
    });
    ...
    test('seed_version stamp = 1 μετά το onCreate', () async {
      final seed = await (db.select(db.userSettings)
            ..where((s) => s.key.equals('seed_version')))
          .getSingleOrNull();
      expect(seed?.value, '1');
    });

    test('FK violation ρίχνει SqliteException (foreign_keys ON)', () async {
      await expectLater(
        db.into(db.receipts).insert(ReceiptsCompanion.insert(
          receiptNumber: 999999,
          receiptDate: DateTime.now(),
          supplierId: 999, // ανύπαρκτος supplier → FK violation
    ...
    test('createdAt αποθηκεύεται ως TEXT (storeDateTimeAsText)', () async {
      final result = await db
          .customSelect('SELECT typeof(created_at) AS t FROM categories LIMIT 1')
          .get();
      expect(result.first.data['t'], 'text');
    });

    test('close() είναι ασφαλές να κληθεί δύο φορές (memory executor)', () async {
      final closedDb = AppDatabase.test();
      await closedDb.close();
      await closedDb.close(); // idempotent — σημαντικό για teardown & restore flow
    });
  });
}
